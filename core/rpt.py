...
# def get_invoice_details(frm_id = None):
#     # tbl = get_object_or_404(tblPurchase_Master, id=frm_id) if frm_id else tblPurchase_Master.objects.last()
#     tbl = tblPurchase_Master.objects.filter(id=frm_id)
#     tbl_details = tblPurchase_Details.objects.filter(purchase = tbl[0])
#     details = []
#     details.append(tbl.values())
#     # print(details)
#     # for detail in tbl_details:
#     details.append(tbl_details)
#     print(details)
...
# def invoice_print(request, frm_id):
#     # Fetch invoice data from database
#     invoice_data = get_invoice_details(frm_id)

#     # Generate HTML content with Jinja2
#     template = Template("template/reports/invoice")
...
#     # Use pyhtml2pdf converter to generate PDF bytes
#     converter = converter(html=soup)
#     pdf_bytes = converter.convert()

#     # Save and return the PDF data
#     c.draw(pdf_bytes)
#     pdf_data = c.stringIO.getvalue()
...
#     with open('base/static/img/main_partner-image.png', 'rb') as f:
#         image_data = f.read()
#     loogo = Image.open(image_data)
#     coolor = colors.Color(113 / 255, 75 / 255, 103 / 255, 0.8)
...
#             ImageReader(loogo), 0.25 * thickness, -2.05 * thickness, 40, 40, mask='auto', preserveAspectRatio=True)
...
# class PdfFileReader(PdfFileReader):
...
# class PdfFileWriter(PdfFileWriter):
...
#     def embed_attachment(self, attachment, subtype=None):
#         assert attachment, "embed_attachment cannot be called without attachment."
...
#         # with open('tools/data/files/sRGB2014.icc', mode='rb') as icc_profile:
